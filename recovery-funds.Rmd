...
set_urbn_defaults(style = "print", base_size = 12, base_family = "Merriweather")
...
# Use showtext to reliably render Merriweather in static (PNG) plots
library(showtext)
...
# Stop the script if 's3_bucket' is not defined
if (s3_bucket == "") {
  stop("Environment variable 's3_bucket' must be defined in your .Renviron file for this code to work. Please update 's3_bucket' in .Renviron and try again.")
}
...
latest_slfrf_year <- 2025
latest_slfrf_qtr <- "Q3"
```

<center>**Last Updated:** `r cur_date`</center>
<br>
<center>**These data are preliminary and subject to revision.**</center>
...
This dashboard tracks a subset of the federal recovery funds flowing into the City of Chicago, Cook County, and the State of Illinois that can be used for inclusive recovery efforts that align with key priorities of The Chicago Community Trust: community investment, community safety, household investment, housing, workforce development, and capacity building, which are all vital to narrowing the Chicago region’s racial and ethnic wealth gap. The dashboard primarily tracks appropriations, or how much funding the federal government has committed to provide local jurisdictions. For the American Rescue Plan Act State and Local Fiscal Recovery Funds (ARPA SLFRF), we are able to track recipient jurisdiction’s adopted budgets, obligations, and expenditures. The flow of recovery funds captured in the dashboard is shown below.

In this dashboard, “allocations” in charts and tables refer to funds originally designated for specific uses in local government plans, such as Covid-19 recovery plans, many of which were drafted in 2021-2022. As community needs and government priorities shifted over time, program allocations may have also shifted, and may vary from the final dollar amount committed to in a local government’s final adopted budget. “Adopted budget” amounts are derived from official local government reporting to the Department of the Treasury.


```{r flowchart, echo=FALSE, out.width="100%", fig.align='center'}
knitr::include_graphics(here::here("www", "images", "Funding Flows Charts_rec.png") )
```

For a few select programs where information is available, the dashboard also explores spending by zip code.We will add more program allocation and spending information as new data become available. For more information, including the **glossary and frequently asked questions**, see [About the Dashboard]{#sec-about} below.
...
data_all <- read_csv(
  "data/all_program_data - updated 3.2026 IR.csv",
  ...
  mutate(
    ...
    is_expired = if_else(expiration > Sys.Date(), 0, 1),
...
write_csv(data_all, file.path(intermediate_path, "clean_data_dashboard.csv"))
```
...
# Helper to convert empty to zero
empty_to_zero <- function(x) {
  if (length(x) == 0) 0 else x
}

pct_il_unexp_b <- empty_to_zero(il_unexp_b) / il_state_allocation_b
pct_chicago_unexp_b <- empty_to_zero(chicago_unexp_b) / chicago_allocation_b
pct_cook_county_unexp_b <- empty_to_zero(cook_county_unexp_b) / cook_county_allocation_b
```
...
```{r c2, fig.width = 10, fig.height = 6, fig.alt= "Bar chart of federal recovery allocations by legislation and policy topic for Cook County showing largest allocation to state and local discretionary funds, followed by housing programs."}
data_filter <- data %>%
...
```{r c3, fig.width = 10, fig.height = 6, fig.alt= "Bar chart of federal recovery allocations by legislation and policy topic for Illinois showing largest allocation to state and local discretionary funds, followed by housing programs."}
data_filter <- data %>%
...
## Difference Between 2021 Allocations and 2024 Adopted Budgets
...
budget_gap_filter <- filter(budget_gap, geography == "Chicago") %>%
  mutate(percent_difference = (adopted_budget_per_treasury_report - allocation) / allocation * 100)

budget_gap_long_filter <- filter(budget_gap_long, geography == "Chicago")

p <- ggplot(budget_gap_filter, aes(x = budget_to_allocation_gap, y = topic)) +
  geom_col(aes(text = paste("Original allocation: ", scales::dollar(allocation), 
                            "<br>Adopted budget: ", scales::dollar(adopted_budget_per_treasury_report), 
  ...
  theme(
    ...
    axis.title = element_text(family = "Merriweather", size = 10)
  )
...
p <- ggplot(budget_gap_filter, aes(x = budget_to_allocation_gap, y = topic)) +
  geom_col(aes(text = paste(
    "Original Allocation: ", scales::dollar(allocation), 
    "<br>Adopted budget: ", scales::dollar(adopted_budget_per_treasury_report), 
    "<br>Difference between allocation and adopted budget: ", scales::dollar(budget_to_allocation_gap),
    "<br>Percent difference between allocation and adopted budget: ", round(percent_difference, 2), "%"
  )), fill = "#47c3d3") +
  scale_x_continuous(
...
```{r adopted-budget-footnote}
adopted_budget_footnote <- 'Adopted budget figures are sourced from Department of Treasury quarterly ARPA SLFRF reports. As of 2025, Treasury reports no longer feature an "Adopted Budget" column. Our data reflects adopted budgets as of Q4-2024.'
```
...
plot_expended_slfrf <- function(.data, geography) {
  .data |>
    ...
    mutate(pct_expended = total_cumulative_expenditures / adopted_budget_per_treasury_report,
           difference = adopted_budget_per_treasury_report - total_cumulative_expenditures) |>
    ...
    coord_flip() +
    scale_y_continuous(labels = scales::label_percent(), limits = c(0, 1.1), breaks = seq(0, 1.1, by = 0.2)) +
    labs(
      x = NULL,
      y = NULL
    ) +
    theme(
      text = element_text(family = "Merriweather", size = 10),          # Set default font
      axis.text = element_text(family = "Merriweather", size = 10),  # Set axis text font and size
      axis.title = element_text(family = "Merriweather", size = 10)  # Set axis title font and size
    ) -> p2

  ggplotly(p2, tooltip = "text", height = 450) %>%
  layout(font = list(family = "Merriweather", size = 10))
}
```
...
# Helper for parsing dollar sign numbers
parse_num_safe <- function(x) {
 ...
 #Read in quarterly data from Excel file
ts_data_in <- readxl::read_excel(
  here::here("data", "CCT - Quarterly ARPA Spending up to Q3 2025.xlsx"),
  sheet = "All"
) %>%
  janitor::clean_names() %>%
  mutate(
    dashboard_project_name = stringr::str_to_title(dashboard_project_name),
    project_name = stringr::str_to_title(project_name)
  )

# Quarterly data cleanup
ts_data <- ts_data_in %>%
  mutate(
    dashboard_topic = case_when(
      dashboard_topic %in% c("Household Investements", "Household Investment") ~ "Household Investments",
      TRUE ~ dashboard_topic
    ),
    dashboard_project_name = na_if(trimws(dashboard_project_name), ""),
    project_name = na_if(trimws(project_name), "")
  ) %>%
  filter(
    trimws(completion_status) != "Cancelled",
    trimws(funding_source) == "ARPA SLFRF"
  ) %>%
  mutate(
    across(
      c(adopted_budget, total_obligations, total_expenditures, percent_of_expended_budgeted_funds),
      ~ readr::parse_number(as.character(.x))
    ),
    dashboard_project_name = coalesce(dashboard_project_name, project_name)
  ) %>%
  filter(!is.na(dashboard_project_name)) %>%
  arrange(dashboard_project_name, desc(report_period))

# Check for duplicate project_id/report_period combinations
dup_keys <- ts_data %>%
  count(project_id, report_period) %>%
  filter(n > 1)

if (nrow(dup_keys) > 0) {
  stop("Duplicate project_id/report_period combinations found in ts_data. Please correct before continuing")
}

# Create sorted report period levels
report_period_levels <- sort(unique(ts_data$report_period))

ts_filled_data <- ts_data %>%
  select(
    project_id,
  ...
  mutate(
    report_period = factor(report_period, levels = report_period_levels, ordered = TRUE)
  ) %>%
  group_by(project_id) %>%
  tidyr::complete(
    report_period = factor(report_period_levels, levels = report_period_levels, ordered = TRUE)
  ) %>%
  arrange(report_period, .by_group = TRUE) %>%
  tidyr::fill(
    dashboard_project_name,
    geography,
    dashboard_topic,
  ...
  ungroup() %>%
  arrange(project_id, report_period)

ts_filled_summary <- ts_filled_data %>%
  group_by(dashboard_topic, geography, report_period) %>%
  summarise(total_expenditures = sum(total_expenditures, na.rm = TRUE), .groups = "drop") %>%
  arrange(geography, dashboard_topic, report_period)

ts_data_with_budget <- ts_data %>%
  left_join(
    data_all %>%
      select(project_id, adopted_budget_per_treasury_report) %>%
      distinct(project_id, .keep_all = TRUE),
    by = "project_id"
  ) %>%
  mutate(
    adopted_budget = coalesce(
      adopted_budget_per_treasury_report,
      adopted_budget
    )
  )
  
dashboard_topic_budgets <- ts_data_with_budget %>%
  group_by(project_id) %>%
  arrange(report_period, .by_group = TRUE) %>%
  tidyr::fill(adopted_budget, geography, dashboard_topic, dashboard_project_name, .direction = "down") %>%
  slice_tail(n = 1) %>%
  ungroup() %>%
  select(project_id, dashboard_project_name, geography, adopted_budget, dashboard_topic) %>%
  filter(!is.na(dashboard_topic), !is.na(geography)) %>%
  group_by(dashboard_topic, geography) %>%
  summarise(
    total_adopted_budget = sum(adopted_budget, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  arrange(dashboard_topic, geography)

data_ts <- ts_filled_summary %>%
  left_join(dashboard_topic_budgets, by = c("dashboard_topic", "geography")) %>%
  rename(topic = dashboard_topic) %>%
  mutate(
    date = zoo::as.yearqtr(as.character(report_period), format = "%Y-Q%q"),
    pct_expended_budget = dplyr::if_else(
      !is.na(total_adopted_budget) & total_adopted_budget != 0,
      total_expenditures / total_adopted_budget,
      NA_real_
    )
  )

# Define custom color palette
custom_colors <- c("#47c3d3", "#C1D82F", "#6C5893", "#000000", "#387ECF", "#FDBB30")

# Plot function with custom colors and fixed axis
plot_spending_by_quarter <- function(.data, geography) {
  .data %>%
    ...
    scale_x_yearqtr(
      format = "%Y-Q%q",
      breaks = seq(from = min(.data$date), to = max(.data$date), by = 0.25)
    ) +
    scale_y_continuous(
    ...
    theme_minimal() +
    theme(
      text = element_text(family = "Merriweather", size = 11),
      axis.text = element_text(family = "Merriweather", size = 11),
      ...
      axis.title = element_text(family = "Merriweather", size = 11),
      legend.text = element_text(family = "Merriweather", size = 11),
      legend.position = "bottom"
    )
}

# Nest and prepare for tabs
nested_data <-
  data_ts %>%
  select(geography, date, topic, pct_expended_budget) %>%
  arrange(geography) %>%
  nest(.by = c(geography), .key = "ts") %>%
  mutate(ts_plots = map(ts, ~ plot_spending_by_quarter(.x)))
```
...
<div class="tabset">
<!-- Tab for each geography -->
```{r quarterly-spending-footnote}
quarterly_spending_footnote <- "In some cases, expenditure data from Department of Treasury quarterly ARPA SLFRF reports show a decrease in expenses between quarters for specific programs. This may be a reporting or clerical error, or may reflect funds that jurisdictions redistributed to other programs."
```

```{r qtr_loop, echo=FALSE, results='asis', fig.width=10, fig.height=6}
for (geo in unique(nested_data$geography)) {
  cat('\n\n### ', geo, '\n')
  print(nested_data$ts_plots[[which(nested_data$geography == geo)]])
  cat('\n\n', quarterly_spending_footnote, '\n')
}
```
</div>
...
plot_funds_by_geo <- function(data, fill_var) {
  ...
  allocations_by_geo <- data %>%
    ...
    theme(
      text = element_text(family = "Merriweather"),          # Set default font for all text
      legend.text = element_text(family = "Merriweather", size = 10), # Legend text
      plot.caption = element_text(family = "Merriweather", hjust = 0, size = 8.5), # Caption font
      plot.caption.position = "plot" # Position of the caption
    )
