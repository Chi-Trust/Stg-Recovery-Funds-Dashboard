...
ts_filled_data <- ts_data %>%
  ...
  ungroup() %>%
  arrange(geography, dashboard_project_name, report_period)
...
dashboard_topic_budgets <- ts_data %>%
  group_by(project_id) %>%
  arrange(report_period, .by_group = TRUE) %>%
...
plot_spending_by_quarter <- function(.data, geography) {
  .data %>%
    ...
    scale_x_yearqtr(format = "%Y-Q%q", breaks = seq(from = min(data_ts$date), to = max(data_ts$date), by = 0.25)) +
    scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, by = 0.2), labels = scales::label_percent()) +
    scale_color_manual(values = custom_colors) +
    labs(x = NULL, y = NULL, color = NULL) +  # Remove 'color' legend title
